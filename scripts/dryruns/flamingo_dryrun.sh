#!/usr/bin/env bash
# Dryrun smoketest for FlamingoChessLM training. Tests the full pipeline
# (init, train steps, eval, checkpoint save) with minimal compute.
# Run with:
#   TEST_MODE=1 srun --partition=pli-c --gres=gpu:1 --mem=64G --time=00:20:00 bash chesslm/scripts/dryruns/flamingo_dryrun.sh
set -xeuo pipefail
