#!/usr/bin/env bash
# Dryrun smoketest for KVProjChessLM (full decoder) training. Tests the full pipeline
# (init, train steps, eval, checkpoint save) with minimal compute.
# Run with:
#   TEST_MODE=1 srun --partition=pli-c --gres=gpu:1 --mem=128G --time=00:20:00 bash chesslm/scripts/dryruns/kv_proj_dryrun.sh
set -xeuo pipefail
...
TEST_MODE=1 python -m chesslm.train \
    --arch                kv_proj \
    --proj-mode           interleaved \
    --lora-rank           16 \
    --exp-name            kv_proj_dryrun \
